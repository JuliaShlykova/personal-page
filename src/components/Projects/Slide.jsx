    ...
    <li className={classNames('slide', { active })}>
      <div className="video-wrapper" aria-hidden="true">
        <video ref={videoRef} loop>
          <source
            src={`${import.meta.env.BASE_URL}/${project['video-url']}`}
            type="video/mp4"
          />
        </video>
      </div>
      <div className="slide-header">
        <h3 ref={(node) => (node ? preventPropagation(node) : null)}>
          {project.name}
        </h3>
        <div
          className="links"
        ...
        >
          <a href={project['demo-url']} title="visit">
            <RiExternalLinkFill />
          </a>
          <a href={project['github-url']} title="github">
            <FaGithub />
          </a>
        </div>
      </div>
      ...
      <ul className="tools">
        {project.tools.map((tool, i) => (
          <li key={i} title={tool}>
            <img
              src={`${import.meta.env.BASE_URL}/icons-tools/${tool.toLowerCase()}.svg`}
              alt={tool}
            />
          </li>
        ))}
